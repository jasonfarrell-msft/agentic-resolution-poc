...
- **Dev vs Test Container App topology gap (2026-05-08):** Setup-Solution.ps1 deploys **exactly 2 container apps** (`ca-api-{env}` + `ca-res-{env}`) regardless of environment. Test environment (rg-agent-resolution-test) has this correct topology. Dev environment (rg-agentic-res-src-dev) has **7 container apps total** including `ca-mcp-tocqjp4pnegfo`, `ca-classifier-tocqjp4pnegfo`, `ca-incident-tocqjp4pnegfo`, `ca-request-tocqjp4pnegfo`, and `ca-escalation-tocqjp4pnegfo` — these 5 additional containers are NOT provisioned by Setup-Solution.ps1. Root cause: Manual/ad-hoc deployments to dev during the hosted agent migration phase (2026-05-04) when Bishop scaffolded Python-based hosted agents. These agents were deployed directly via Azure CLI to dev but never automated into the setup script. The hosted agents architecture was later replaced by the current Python Resolution API (`ca-resolution`) that orchestrates agents internally. The setup script intentionally deploys only the two production-topology containers (CRUD API + Resolution API); MCP server and per-agent containers remain as 0-byte stub Bicep modules (`infra/modules/containerapp-mcp.bicep`, `infra/modules/containerapp-agent.bicep`) pending Phase 2+ architecture decisions. Dev environment should be cleaned up to match test topology or the stale containers documented as development-only experimental deployments.
...
### 2026-05-08: Container App Topology Gap Investigation
- **Question:** Why does dev have a container app deployed for each agent, but test only has 2 container apps?
- **Finding:** **Setup-Solution.ps1 deploys exactly 2 container apps to all environments** (Tickets API + Python Resolution API). No environment branching.
- **Dev Extras Explained:** Manual deployments from hosted-agents migration experiment (ca-mcp, ca-incident, ca-classifier, ca-request, ca-escalation, ca-resolution). Not part of current scripted path.
- **Test Correct:** 2 container apps (ca-api-agent-resolution-test, ca-res-agent-resolution-test) match scripted deployment. ✅
- **Action:** Recommend re-provisioning dev via setup script to restore parity (2 containers).
- **Phase 2 Decision:** Team votes on three topology options (keep 2 containers, add MCP Server, or environment-specific). Decision documented in `.squad/decisions.md` (2026-05-08).
- **agent-resolution-test2 deployment (2026-05-08):** Deployed fresh environment in eastus2 from clean slate. Infrastructure provisioned successfully (Azure SQL, Key Vault, Container Apps Environment, Container Registry, Web App, 2 Container Apps). Setup script hung during ACR build step but all resources were created. Post-deployment fixes required: (1) Managed identity database users configured via Configure-DatabaseUsers.ps1 using correct identity names (id-api-agent-resolution-test2, not ca-api-agent-resolution-test2); (2) Admin API key secret created and configured in Container App with environment variable `AdminEndpoints__ApiKey=secretref:admin-api-key` (not `AdminApiSettings__AdminApiKey`); (3) Container App revision restarted to apply configuration changes. Final validation: Web App healthy at app-agent-resolution-test2-web.azurewebsites.net; Tickets API healthy at ca-api-agent-resolution-test2.delightfulbay-f32d8642.eastus2.azurecontainerapps.io with 15 seeded tickets (INC0010015 through INC0010029); Resolution API healthy at ca-res-agent-resolution-test2.delightfulbay-f32d8642.eastus2.azurecontainerapps.io. Lesson: Setup-Solution.ps1 has a hang issue during `az acr build` (likely timeout or progress display issue); all resources provision correctly but script doesn't complete gracefully. Manual post-deployment steps (database users, admin key) should be integrated into a retry/completion path.


### 2026-05-08: test2 Resolution API Azure OpenAI RBAC Fix
- **Issue:** Resolve with AI failed because principal `c6b82506-1e92-49b1-8e4b-962defc93a9f` lacked `Microsoft.CognitiveServices/accounts/OpenAI/deployments/chat/completions/action`.
- **Identity:** The principal is the user-assigned managed identity `id-resolution-agent-resolution-test2` attached to `ca-res-agent-resolution-test2` in `rg-agent-resolution-test2`.
